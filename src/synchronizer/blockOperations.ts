...
export class BlockOperations {
  ...
  async initbucket(caller: string, height: number, hash: string, block_size: number, num_chunks: number) {
    ...
    if (result) {
      logger.info(
        `[${caller}] initbucket success, height: ${height}, hash: ${hash}, transaction_id: ${result.transaction_id}`
      );
  ...
  async delbucket(caller: string, height: number, hash: string) {
    ...
    if (result) {
      logger.info(
        `[${caller}] delbucket success, height: ${height}, hash: ${hash}, transaction_id: ${result.transaction_id}`
      );
      blockUploadTotalCounter.inc({
        account: this.accountName,
        client: Client.Synchronizer,
  ...
  async pushchunk(caller: string, height: number, hash: string, chunkId: number, chunkData: string) {
    ...
    if (result) {
      logger.info(
        `[${caller}] pushchunk success, height: ${height}, hash: ${hash}, chunk_id: ${chunkId}, transaction_id: ${result.transaction_id}`
      );
      blockUploadTotalCounter.inc({
        account: this.accountName,
        client: Client.Synchronizer,
