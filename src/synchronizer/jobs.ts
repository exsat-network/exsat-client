...
export class SynchronizerJobs {
  constructor(
    public state: SynchronizerState,
    ...
    try {
      ...
      if (!blockbuckets || blockbuckets.length === 0) {
        ...
        if (uploadingBlockbucket) {
          const { bucket_id, height, hash, size, uploaded_size, num_chunks, uploaded_num_chunks, chunk_size } =
        ...
        if (usedSlots < holdSlots) {
          ...
          if (forkHeight > 0) {
            ...
            logger.info(
              `The number of blockbuckets[${usedSlots}] has reached the upper limit[${holdSlots}], Please purchase more slots or wait for the slots to be released`
            );
            await sleep(5000);
          }
        }
  ...
  verify = async () => {
    ...
    try {
      ...
      let verifyBucket;
      for (const blockbucket of blockbuckets) {
        const consensusblk = await this.state.tableApi!.getConsensusByBlockId(
          BigInt(blockbucket.height),
          blockbucket.hash
        );
        if (consensusblk || chainstate!.irreversible_height >= blockbucket.height) {
          logger.info(
            `delbucket: The block has been endorsed, height: ${blockbucket.height}, hash: ${blockbucket.hash}`
          );
          await this.blockOperations.delbucket(caller, blockbucket.height, blockbucket.hash);
        } else {
          if (!verifyBucket) {
      ...
      switch (status) {
        ...
        case BlockStatus.VERIFY_PASS:
          logger.info(
            `Block verify pass, Wait for the validator to endorse the block, height: ${height}, hash: ${hash}`
          );
          break;
        default:
          break;
  ...
  parse = async () => {
    ...
    try {
      ...
      for (const item of chainstate!.parsing_progress_of) {
        ...
        if (
          parseInfo.parser === this.state.accountName ||
          moment.utc().isAfter(moment.utc(parseInfo.parse_expiration_time))
        ) {
